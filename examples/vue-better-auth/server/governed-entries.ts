...
const DIGEST_KEYS = { keyedDigest: { keyVersion: "example-key-1", secret: "vue-example-keyed-digest-secret" } };

const PRODUCER: EvidenceRef = {
  authority: "veritio.example",
  kind: "principal",
  type: "service",
  id: "vue-governed-crud",
};
...
function outbox(): OutboxAdapter {
  ...
  ref.__veritioGovernedOutbox ??= createFileOutboxAdapter(join(tmpdir(), "veritio-vue-example-outbox"));
 ...
 * across tsx-watch server reloads so a restart does not change ids mid-run.
 */
...
export async function runGovernedAction(input: GovernedActionInput): Promise<GovernedActionResult> {
  ...
  // the transactional-outbox boundary (design §7.1/§7.4). The enqueue can throw
  // (e.g. an idempotency conflict on a re-used change id); if it does, the local
  // entry store must stay unchanged so a retry sees the same before-state rather
  // than a half-advanced entry that reports "no governed fields changed".
