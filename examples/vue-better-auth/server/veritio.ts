 ...
 * activity-graph edge. Project state stays local to the example, while tenant
 * and actor identity stay server-resolved.
 */
 ...
 * show audit records and graph edges together.
 */
 ...
 * template helpers own event semantics; the host example owns graph assembly.
 */
...
/**
 * Mutates the local project map before evidence is emitted so API responses and
 * Veritio records describe the same resource state.
 */
 ...
 * hashing and envelope hashing for local graph verification.
 */
...
/**
 * Maps CRUD verbs to language-neutral audit action names shared across
 * framework examples.
 */
...
/**
 * Maps CRUD verbs to supported evidence graph relations instead of creating
 * framework-specific protocol terms.
 */
