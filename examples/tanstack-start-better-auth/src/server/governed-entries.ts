 ...
 * Identity/tenant are server-owned; the browser never supplies them.
 */
...
const DIGEST_KEYS = { keyedDigest: { keyVersion: "example-key-1", secret: "tanstack-example-keyed-digest-secret" } };

const PRODUCER: EvidenceRef = {
  authority: "veritio.example",
  kind: "principal",
  type: "service",
  id: "tanstack-governed-crud",
};
...
/** Reuses one entry store across Vite dev reloads. */
...
/** Reuses one change feed (newest first) across Vite dev reloads. */
...
/** Reuses one durable file outbox across Vite dev reloads. */
function outbox(): OutboxAdapter {
  const ref = globalThis as typeof globalThis & { __veritioGovernedOutbox?: OutboxAdapter };
  ref.__veritioGovernedOutbox ??= createFileOutboxAdapter(join(tmpdir(), "veritio-tanstack-example-outbox"));
 ...
 * across Vite dev reloads so HMR does not change ids mid-run.
 */
...
export async function runGovernedAction(input: GovernedActionInput): Promise<GovernedActionResult> {
  ...
  // the transactional-outbox boundary (design §7.1/§7.4). Ordering it this way
  // means a rejected enqueue (e.g. an idempotency conflict) never leaves the
  // in-memory entry partially advanced to a new revision.
