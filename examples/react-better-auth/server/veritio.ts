...
/**
 * Applies a project CRUD mutation and records both a Veritio audit event and an
 * activity-graph edge. The example keeps project state in memory so the API is
 * runnable without hosted storage, while tenant and actor identity stay
 * server-resolved.
 */
...
/**
 * Returns a verified evidence snapshot for the reference tenant so the UI can
 * show how CRUD audit events and graph edges compose into one governed trail.
 */
 ...
 * subject-request, export, retention, and processor-transfer evidence. This
 * uses SDK template helpers for event semantics and keeps the graph assembly at
 * the host boundary.
 */
...
/**
 * Mutates the local project map before evidence is emitted so the audit record
 * reflects the same state the API returns to the browser.
 */
...
/**
 * Appends a tenant-scoped evidence-edge record with deterministic idempotency
 * hashing and hash-chain metadata. This mirrors production storage semantics
 * without requiring a hosted Veritio account.
 */
...
/**
 * Maps CRUD verbs to language-neutral audit action names shared across the
 * reference examples.
 */
...
/**
 * Maps CRUD verbs to supported evidence graph relations without inventing
 * framework-specific protocol vocabulary.
 */
 ...
 * use the same deterministic record envelope as CRUD edges.
 */
 ...
 * existing entity and relation vocabulary.
 */
function scenarioEdge(
  ...
  return {
    ...
    metadata: {
      source: "react-governed-lifecycle",
      canonicalPlanHash,
    },
  };
}
