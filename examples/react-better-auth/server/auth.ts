...
function getLocalBetterAuthSecret(): string {
  const referenceGlobal = globalThis as typeof globalThis & {
    __veritioReactBetterAuthSecret?: string;
  };
  referenceGlobal.__veritioReactBetterAuthSecret ??= `local-react-${randomUUID()}`;
  return referenceGlobal.__veritioReactBetterAuthSecret;
}
