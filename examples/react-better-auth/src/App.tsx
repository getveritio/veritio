...
export function App() {
  ...
  // AbortController is the sanctioned effect use (see rule 08). Re-reads after an
  // action go through `refresh()` from the event handler, not an effect.
  useEffect(() => {
  ...
  async function refresh(signal?: AbortSignal) {
    try {
      ...
      if (!response.ok) {
        setLoadError("Could not load the governed snapshot. Is the Express server running on :3001?");
        return;
      }
      setSnapshot((await response.json()) as GovernedSnapshot);
      setLoadError(null);
    } catch (cause) {
      if ((cause as Error)?.name === "AbortError") return;
      setLoadError("Could not reach the Express server on :3001. Start it with `bun run dev:server`.");
    }
  }
  ...
  async function act(input: GovernedActionInput) {
    ...
    try {
      ...
      setLast((await response.json()) as GovernedActionResult);
      await refresh();
    } catch {
      setError("The governed action could not reach the Express server.");
    } finally {
      setBusyId(null);
    }
  }

  /** Triggers one governed agent session, then re-reads the snapshot. */
  async function runSession() {
    ...
    try {
      ...
      setError("The agent session could not reach the Express server.");
    } finally {
  ...
  if (!snapshot) {
    return (
      <div className="grid min-h-screen place-items-center bg-dotgrid">
        <p className="text-sm text-muted-foreground">{loadError ?? "Loading governed snapshot…"}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-dotgrid">
      <Topbar cloud={snapshot.cloud} />
      <main className="mx-auto max-w-6xl space-y-8 px-6 py-8">
        <Intro cloud={snapshot.cloud} />
      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center gap-3 px-6">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-[11px] text-muted-foreground">
            React + Express reference — edit → capture → outbox → hosted ingest
          </p>
        </div>
      ...
      <CardContent className="space-y-2 p-5 text-sm text-muted-foreground">
        ...
        {cloud.configured ? (
          ...
          <p>
            ...
            the Cloud console) on the Express server and restart to dispatch end-to-end.
          </p>
        )}
      </CardContent>
    </Card>
      ...
      {sessions.length === 0 ? (
        ...
        <Card className="overflow-hidden">
          {sessions.map((session) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate font-mono text-[11px] text-muted-foreground">{session.sessionId}</p>
