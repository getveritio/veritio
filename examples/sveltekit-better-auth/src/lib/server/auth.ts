...
/**
 * Provides the reference tenant boundary for the SvelteKit Better Auth handler.
 * Real apps must replace this with Better Auth session plus organization
...
/**
 * Exposes the local Better Auth instance mounted by SvelteKit hooks. This is an
 * example host boundary, not a protocol authority.
 */
...
function getLocalBetterAuthSecret(): string {
  const referenceGlobal = globalThis as typeof globalThis & {
    __veritioSvelteKitBetterAuthSecret?: string;
  };
  referenceGlobal.__veritioSvelteKitBetterAuthSecret ??= `local-sveltekit-${randomUUID()}`;
  return referenceGlobal.__veritioSvelteKitBetterAuthSecret;
}

/**
 * Reads optional request correlation headers from Better Auth context without
 * using browser-controlled tenant identity.
 */
...
/**
 * Extracts Headers from direct or request-wrapped hook context shapes.
 */
