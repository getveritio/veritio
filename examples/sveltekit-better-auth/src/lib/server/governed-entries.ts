 ...
 * lives under `$lib/server` (SvelteKit never bundles it to the client) and is the
 * only place that reads cloud env config.
 */
...
const DIGEST_KEYS = { keyedDigest: { keyVersion: "example-key-1", secret: "svelte-example-keyed-digest-secret" } };

const PRODUCER: EvidenceRef = {
  authority: "veritio.example",
  kind: "principal",
  type: "service",
  id: "svelte-governed-crud",
};
...
/** Reuses one entry store across Vite/HMR server reloads. */
...
/** Reuses one change feed (newest first) across Vite/HMR server reloads. */
...
/** Reuses one durable file outbox across Vite/HMR server reloads. */
function outbox(): OutboxAdapter {
  const ref = globalThis as typeof globalThis & { __veritioGovernedOutbox?: OutboxAdapter };
  ref.__veritioGovernedOutbox ??= createFileOutboxAdapter(join(tmpdir(), "veritio-sveltekit-example-outbox"));
 ...
 * across Vite/HMR server reloads so HMR does not change ids mid-run.
 */
...
export async function runGovernedAction(input: GovernedActionInput): Promise<GovernedActionResult> {
  ...
  // the transactional-outbox boundary (design §7.1/§7.4). Enqueue-first keeps the
  // two in lockstep: if the enqueue throws (e.g. an idempotency conflict on a
  // reused change id) the in-memory entry is left untouched, so a retry does not
  // see a half-advanced row reporting "no governed fields changed".
