 ...
 *
 * The hosted ingest API has no CORS, so delivery is server-to-server: an App
 * Router server action computes a governed change and dispatches the outbox to
 * the Cloud from the Node runtime. Per the repo rules, the scoped ingest key and
 * base URL are read from environment ONLY here (a process-boundary module),
 * never in SDK core and never shipped to the browser. The browser only ever sees
 * `cloudPublicConfig()` (no token). The `server-only` import makes an accidental
 * client import a build-time error.
 *
 * Configure (e.g. in `.env.local` / the dev shell):
