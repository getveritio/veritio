 ...
 * is `server-only` and is the only place (with cloud-ingest) that reads cloud env
 * config — App Router server actions call into it from the Node runtime.
 */
...
const DIGEST_KEYS = { keyedDigest: { keyVersion: "example-key-1", secret: "nextjs-example-keyed-digest-secret" } };

const PRODUCER: EvidenceRef = {
  authority: "veritio.example",
  kind: "principal",
  type: "service",
  id: "nextjs-governed-crud",
};
...
/** Reuses one entry store across Next dev-server reloads. */
...
/** Reuses one change feed (newest first) across Next dev-server reloads. */
...
/** Reuses one durable file outbox across Next dev-server reloads. */
function outbox(): OutboxAdapter {
  const ref = globalThis as typeof globalThis & { __veritioGovernedOutbox?: OutboxAdapter };
  ref.__veritioGovernedOutbox ??= createFileOutboxAdapter(join(tmpdir(), "veritio-nextjs-example-outbox"));
 ...
 * across Next dev-server reloads so HMR does not change ids mid-run.
 */
...
export async function runGovernedAction(input: GovernedActionInput): Promise<GovernedActionResult> {
  ...
  // the transactional-outbox boundary (design §7.1/§7.4). Enqueue-first is the
  // fail-closed order: if the enqueue throws (e.g. an idempotency conflict on a
  // re-used change id), the in-memory entry must stay at its prior revision.
