...
/**
 * shadcn new-york button variants, copied 1:1 from veritio-cloud so example
 * apps render identical ink-on-zinc buttons. `default` is near-black --primary
 * ink (NOT a brand hue); emerald is reserved for --success elsewhere. Keep in
 * sync with veritio-cloud/src/components/ui/button.tsx (README SYNC NOTE).
 */
...
/**
 * Polymorphic button. `asChild` renders the single child as the button via
 * Radix Slot so links and menu items inherit button styling without nesting.
 */
function Button({
  className,
  variant,
