export interface ReactVeritioTargetRef {
  type: string;
  id: string;
}

export interface ReactVeritioAttributeInput {
  action: string;
  target: ReactVeritioTargetRef;
  purpose?: string;
}

export interface ReactVeritioAttributes {
...
/**
 * Creates inert client-side data attributes for React elements while rejecting
 * tenant scope, recorder, storage, and secret-like fields that belong on servers.
 */
export function createReactVeritioAttributes(input: ReactVeritioAttributeInput): ReactVeritioAttributes {
  ...
  const attributes: ReactVeritioAttributes = {
...
export const createVeritioAttributes = createReactVeritioAttributes;
