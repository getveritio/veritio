...
| `policyDecision` | `"allow" \| "deny" \| "none"` | always; `"none"` = the request ended (e.g. client aborted mid-upload) before policy evaluation ran |
...
**Cost rounding (normative):** `costMicroUsd` rounds half-up per direction
(`round(tokens × microUsdPerMTok ÷ 1e6)`, input and output rounded independently, then summed).
Ports MUST implement explicit half-up rounding — language-default rounding (e.g. Python's
banker's rounding) diverges at exact .5 boundaries.
